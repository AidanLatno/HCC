...
public class SAirChargeProj extends SamuraiChargeProj{
    public SAirChargeProj(LivingEntity caster, float speed, float radius, float swordDamage) {
        super(caster, speed, radius, swordDamage);
    }

    public SAirChargeProj(LivingEntity caster, float speed, float swordDamage) {
    ...
    public void logic(boolean controllable)
    {
        ...
        ParticleMaker.SpawnParticle(getLocation(), Particle.CLOUD,1);
        GeneralUtils.PlaySound(getLocation(), Sound.ENTITY_CREEPER_HURT,1,2);
